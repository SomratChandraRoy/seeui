...
const MIN_AMOUNT = 5; // $5 minimum enforced by Stripe and our backend
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      if (!APPWRITE_FUNCTION_URL) {
        throw new Error("Appwrite function URL is not configured. (Check .env)");
      }
      ...
      const response = await fetch(APPWRITE_FUNCTION_URL, {
        ...
        headers: {
          ...
          'X-Appwrite-Project': APPWRITE_PROJECT_ID || '', // Needed if calling execution endpoint directly
        },
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(155deg, rgba(30,30,40,0.95) 0%, rgba(15,15,22,0.98) 100%)',
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold tracking-tight mb-2">Support SeeUI</h2>
          <p className="text-sm text-slate-400">
            SeeUI is free forever. If it saves you time, consider buying us a coffee to support development!
          </p>
        </div>
        ...
        <div className="mb-6">
          ...
          >
            <div className={`pl-4 font-bold ${isCustom ? 'text-white' : 'text-slate-400'}`}>$</div>
            <input
              type="number"
              min={MIN_AMOUNT}
              step="1"
          ...
          {isCustom && parseFloat(customAmount) < MIN_AMOUNT && (
            <p className="mt-2 text-xs text-rose-400 pl-1">
              Minimum donation is ${MIN_AMOUNT} (Stripe limits)
            </p>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading || (isCustom && parseFloat(customAmount) < MIN_AMOUNT)}
        ...
        >
          <div className="relative z-10 flex items-center justify-center gap-2">
            {loading ? (
              ...
              <>
                <span>Support for ${isCustom ? (customAmount || 0) : amount}</span>
                <Heart size={16} className="fill-white/20 transition-transform group-hover:scale-110" />
              </>
            )}
          </div>
          {/* Glass shine effect */}
